...
func (c *Client) processor() error {
	...
	for {
		...
		switch p := pkt.(type) {
		case *packet.Publish:
			fmt.Printf("received: %s, pid: %d, qos: %d, topic: %s", p.Type(), p.ID, p.Message.QOS, p.Message.Topic)
			if c.handler != nil {
		...
		case *packet.Puback:
			fmt.Printf("received: %s, pid: %d", p.Type(), p.ID)
			if c.handler != nil {
...
func (c *Client) pinger() (err error) {
	fmt.Printf("pinger starting")
	defer fmt.Printf("pinger stopped")
